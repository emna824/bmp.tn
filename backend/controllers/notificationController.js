...
async function createNotification({ userId, type, message, relatedId = null }) {
  ...
  if (!NOTIFICATION_TYPES.includes(String(type || ''))) {
...
async function notifyAssignedArtisansAboutProjectUpdate({ project, event, reason = '' }) {
  const artisanIds = Array.from(
    new Set(
      (Array.isArray(project?.assignedArtisans) ? project.assignedArtisans : [])
        .map((artisan) => toObjectIdString(artisan))
        .filter((artisanId) => isValidObjectId(artisanId))
    )
  );

  if (!project?._id || !artisanIds.length) {
    return [];
  }
  ...
  if (normalizedEvent === 'started') {
    ...
    message = `Project "${project.projectName}" has started. Check your assigned work and schedule.`;
  } else if (normalizedEvent === 'closed') {
    type = 'project_closed';
    message = reason
      ? `Project "${project.projectName}" was closed. ${String(reason).trim()}`
      : `Project "${project.projectName}" was closed.`;
  } else {
    return [];
  }
...
exports.notifyAssignedArtisansAboutProjectUpdate = notifyAssignedArtisansAboutProjectUpdate;
